...
class LearningAgent(Agent):
    ...
    def reset(self, destination=None, testing=False):
        ...
        # Update trail count
        self.trail_count += 1.0
        # self.epsilon = float(1 / self.trail_count ** 2)
        # self.epsilon = math.fabs(math.cos(0.02 * self.trail_count))
        # self.epsilon = 0.02 ** self.trail_count
        self.epsilon = math.exp(-0.02 * self.trail_count)
        if testing:
            self.epsilon = 0
            self.alpha = 0
    ...
    def choose_action(self, state):
        ...
        if not self.learning:
            ...
            if self.epsilon > 0.01 and self.epsilon > random.random():
                ...
                actions = [act for act in self.Q[state] if self.Q[state][act] == self.get_maxQ(state)]

        return random.choice(actions)

    def should_take_random_action(self):
        return not self.learning or self.epsilon > random.random()
